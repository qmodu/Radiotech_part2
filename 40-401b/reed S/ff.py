...
class GF256int(int):
    """Экземпляры этого объекта являются элементами поля GF(2^8)
    экземпляры целые числа в диапазоне от 0 до 255
    Это поле определяется с помощью неприводмого многочлена
    x^8 + x^4 + x^3 + x + 1
    и используя 3 в качестве генератора для экспоненциальной таблицы
    и логарифмической таблицы.
    """
    # Карта целых чисел GF256int случаев
    cache = {}
    # Экспоненциальная таблица для 3, генератор для GF(256)
    ...
    # Логарифмическая таблица, основание 3
    ...
    def __new__(cls, value):
        # Проверяем кэш
        # Кэширование жертвует немного скорости за меньшим использованием памяти.
        # Таким образом, есть максимальное число 256 переменных для этого класса.
        try:
            return GF256int.cache[value]
        except KeyError:
            if value > 255 or value < 0:
                raise ValueError("Элементы поля GF(2^8) находятся между 0 и 255. Не может быть %s" % value)
    ...
    def __add__(a, b):
        "Добавление в GF(2^8) есть сложение по модулю два"
        return GF256int(a ^ b)
    __sub__ = __add__
    __radd__ = __add__
    __rsub__ = __add__
    ...
    def __pow__(self, power):
        if isinstance(power, GF256int):
            raise TypeError("Повышение элемента поля до другого элемента поля не определено. Сила повышения должна быть постоянным целым числом")
    ...
    def multiply(self, other):
        """Медленный метод умножения. Этот метод дает те же результаты что и
        другие методы умножения, но реализцется чтобы проиллюстрировать, как это
        работает и показать как были сгенерированны приведенные выше таблицы.
        """
        a = int(self)
        b = int(other)
